...
export default function Switch({ MainComponent, theme, OtherComponent, mainText, otherText, handleClick, setSelectedPair, options, setPool }){
    ...
    const switchClassName=`rounded-035 md:rounded-lg p-2 md:p-4 block w-1/2 bg-${formSwitch.sliderbg} rounded-lg outline-none focus:outline-none absolute top-0 font-bold text-${formSwitch.sliderTxt} ${translation}`
    return (
      <div className="mx-6 bg-white md:mx-auto mt-5 md:mt-10 md:max-w-lg m-auto shadow-lg rounded-xl md:rounded-2xl md:rounded-b-3xl overflow-hidden">
        <div className={`w-full h-10 md:h-auto ${bgStyle} shadow-md rounded-md md:rounded-xl overflow-hidden border border-${formSwitch.sliderBorder} relative`}>
            ...
            <button data-testid="home-slider" className={switchClassName}>{isMain ? mainText : otherText}</button>
				...
				<div className={`${bgStyle} px-4 md:px-6 pt-4 md:pt-6 pb-3 text-${formSwitch.note} font-sans text-xs bg-black`}>
